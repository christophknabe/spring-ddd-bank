...
- [JUnit 4](https://junit.org/junit4/) as unit testing framework
- [Mockito](https://site.mockito.org/) 4 for injecting errors into the persistence layer for unit tests
- [MulTEx](http://public.bht-berlin.de/~knabe/java/multex/) as Exception Handling and Reporting Framework
...
- Prefer a [Rich Domain Model](https://www.amido.com/blog/anaemic-domain-model-vs-rich-domain-model) over an [Anemic Domain Model](https://martinfowler.com/bliki/AnemicDomainModel.html) by having relevant business logic methods in entity class [Client](src/main/java/de/beuth/knabe/spring_ddd_bank/domain/Client.java).
  This required **Domain Object Dependency Injection** (DODI), but is now done manually by method `Client.provideWith`. (The former automatic DODI on Java 8 by AspectJ compile-time weaving was abandoned, as there were serious configuration issues with AspectJ on Java 11. AspectJ seems to become less popular.)
...
- The analysis class diagram is in file [src/doc/BankModel.pdf](src/doc/BankModel.pdf). Its editable source by [UMLet](https://www.umlet.com/) has extension `.uxf`.
